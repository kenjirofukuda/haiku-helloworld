...
status_t TimeKeeper::InsertSchedule(const ScheduleInfo& schedule)
{
    ...
    if (fWatcher < 0) {
	fWatcher = ::spawn_thread(WatchClients, "watcher", B_NORMAL_PRIORITY, this);
    }
...
{
    TaskInfo* task = (TaskInfo*)fTaskList->RemoveItem(FindTask(client, code));
    status_t sts = B_OK;
    if (task == nullptr) {
	sts = B_ERROR;
	goto bail;
...
TimeKeeper::ExecuteHeadTask()
{
    ...
    status_t sts = target->PostMessage(task->schedule.message, task->schedule.client);
    if (sts != B_OK)
	goto bail;
